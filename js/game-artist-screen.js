...
import gameGenreScreen from './game-genre-screen';

let gameArtistScreen = `<section class="main main--level main--level-artist">
...
gameArtistScreen = createScreenElement(gameArtistScreen);
const playAgainButton = gameArtistScreen.querySelector(`.play-again`);
playAgainButton.addEventListener(`click`, () => renderScreen(welcomeScreen));

const answerButtons = [...gameArtistScreen.querySelectorAll(`.main-answer`)];
for (let item of answerButtons) {
  item.addEventListener(`click`, () => {
    // чтобы сбросить предыдущие ответы, может это лишнее
    const answers = [...gameGenreScreen.querySelectorAll(`.genre-answer-check`)];
    for (let label of answers) {
